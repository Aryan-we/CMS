...
if($row){
    ...
    window.location.href='viewAttendance.php';</script>";
   /* if(isset($_POST['submit']) and !empty($_POST['status'])){
            ...
            foreach($_POST['status'] as $id=>$status){
                ...
                $result=mysqli_query($connect,"update attendance_records set name='$student_name',
                roll_no='$roll_number',status='$status',date='$date' where date='$date';");
                 if($result){
                    $flag=2;
                 }
            }
...
if(isset($_POST['submit']) and !empty($_POST['name']) and !empty($_POST['roll_no']) and !empty($_POST['status'])){
        ...
        if(mysqli_num_rows($result)==0){
            foreach($_POST['status'] as $id=>$status){
                ...
                $result=mysqli_query($connect1,"insert into attendance_record_by_facultyid_".$a." (name,roll_no,status,date) values('$student_name','$roll_number','$status','$date');");
                 if($result){
                    $flag=1;
                 }
                }
    ...
    <div class="container">
       ...
       <a href="home.php"><input type="button" value="Add Student"></a> 
        ...
        <div class="box3">
            ...
            <input type="submit" value="Submit" name="submit" style="width:18%;font-size:20px; float:left; margin:5px;cursor: pointer; padding:10px;">
            </form>
